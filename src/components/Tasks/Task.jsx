...
const Task = ({id, completed, text, list, onRemove, onEdit, setPopup, onCoplete}) => {
    ...
    const editTask = () => {
        setPopup({ questionCallback:(value) => {
            ...
            .then(() => {
                onEdit(list.id, id, value);
            })
            .catch(() => {
                setPopup({ id:Math.random(),
                    error: true,
                    text: 'Не удалось обновить название задачи' });
            })
        },
            input: text,
            id: `${id}${text}`,
    ...
    const changeCompleted = (e) => {
            ...
            .then(({data}) => {
                onCoplete(list.id, id, data.completed);
            })
            .catch(() => {
                setPopup({ id:Math.random(),
                    error: true,
    ...
    return (
        <div className="tasks__item">
            <div className="tasks__checkbox">
                <input onChange={changeCompleted} checked={completed} id={`task-${id}`} type="checkbox"/>
            ...
            <div className='tasks__descr'>{text}</div>
            <div className="tasks__actions">
                <div className="tasks__action"
                onClick={editTask}
                >
                    <svg
                        width="15"
